...
def build_prompt(question: str, chunks: List[Dict]) -> str:
    """
    Build the strict‑grounding prompt exactly as specified in the instructions.
    """
    parts: List[str] = ["You are an expert assistant.", "", "Context chunks:"]
    for c in chunks:
    ...
    parts.extend(
        [
            ...
            "",
            "Instructions:",
            "• Answer strictly with the information in the chunks.",
            "• Cite each chunk ID in square brackets when its content is used.",
            "• If none of the chunks contain the answer, say you don’t have enough info.",
        ]
    )
    return "\n".join(parts)
...
def extract_citations(answer: str) -> List[str]:
    """
    Parse square‑bracket citations (e.g., “[abc‑123]”) from the answer text.
    Returns unique IDs in ascending order.
    """
