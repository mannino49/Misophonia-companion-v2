    ...
    (d.results || []).forEach((s, i) => {
      ...
      card.innerHTML = `
        ...
        <div class="card-body">
          ...
          ${s.doc.doi ? `<a href="https://doi.org/${s.doc.doi}" target="_blank">${s.doc.doi}</a>` : ''}
        </div>`;
...
def index():
    ...
    resp = make_response(html, 200)      # turn the string into a Response
